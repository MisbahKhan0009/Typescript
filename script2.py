...
typescript_snippets = [
    ...
    'function padLeft(value: string, padding: string | number) { if (typeof padding === "number") { return Array(padding + 1).join(" ") + value; } if (typeof padding is "string") { return padding + value; } throw new Error(`Expected string or number, got ${typeof padding}.`); }',
...
# Create and write the TypeScript snippets to the files
for i in range(100, 150):
    filename = os.path.join(output_dir, f'typescript_file_{i + 1}.ts')
    with open(filename, 'w') as file:
        file.write(typescript_snippets[i % len(typescript_snippets)])

print(f"Created 50 additional TypeScript files starting from 'typescript_file_51.ts' in '{output_dir}' directory.")
